...
interface ImageSplitExampleProps extends React.HTMLAttributes<HTMLDivElement> {
  src: string;
  sections?: number;
  offsetStep?: number;
...
export function ImageSplitExample({
  src,
  sections = 9,
  offsetStep = 30,
  ...
  const setupScrollListener = () => {
    ...
    const updateProgress = () => {
      ...
      const end = scrollContainerRect.top + wrapperHeight * viewportThreshold;
      const current = parentRect.top;

      let progress = (current - end) / (start - end);
...
export default function ImageSplitDemo() {
  ...
  return (
    <div
      ref={wrapperRef}
      className="overflow-y-auto h-full w-full max-h-[450px] relative"
    >
