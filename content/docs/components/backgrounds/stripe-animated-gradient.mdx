...
<ComponentPreview
  ...
  v0Link="https://badtz-ui.com/r/stripe-animated-gradient.json"
>
...
<Step>Download the `stripe-shader.zip`.</Step>

You can extract the `stripe-shader.js` file from the zip and place it inside your `components/ui` directory. (The file is too large to display here in the MDX.)

<DownloadButton />

<Step>
  Copy and paste the `stripe-animated-gradient` code into your project.
</Step>
...
<li className="text-sm text-muted-foreground mt-4">
  I didn't create the original effect; I simply adapted it to work with React.
  Full credit goes to:
</li>
<li className="text-sm text-muted-foreground mt-4">
  <MdxLink href="https://stripe.com/jp">Stripe</MdxLink>
</li>
<li className="text-sm text-muted-foreground mt-4">
  <MdxLink href="https://kevinhufnagl.com/">@kevinhufnagl</MdxLink>
</li>
